...
// Implements prd075-sha384sum R1.1-R1.4, R2.1-R2.3, R3.1-R3.2: Compute and
// check SHA-384 message digests using pkg/hashutil for shared formatting and
...
// sha384Config is the HashConfig for SHA-384 digests.
// D1: Algorithm="SHA384", NewHash=sha512.New384, DigestLen=96 (hex length).
...
// sha384sumOptions holds the parsed flag state for a sha384sum invocation.
type sha384sumOptions struct {
	binary bool // -b/--binary: asterisk before filename (R1.3)
	text   bool // -t/--text: default mode, two spaces (R1.3)
	tag    bool // --tag: BSD-style output (R1.4)
...
// buildCheckOptions constructs CheckOptions from sha384sumOptions.
func buildCheckOptions(opts sha384sumOptions) hashutil.CheckOptions {
...
func parseFlags(args []string) (sha384sumOptions, []string) {
	var opts sha384sumOptions
	var files []string
	endOfFlags := false
...
func handleLongFlag(arg string, opts *sha384sumOptions) bool {
	switch arg {
	case "--binary":
		opts.binary = true
	case "--text":
	...
	case "--zero":
		opts.zero = true
	case "--version":
		fmt.Printf("sha384sum (go-unix-utils) %s\n", version)
		os.Exit(0)
	case "--help":
		printHelp()
		os.Exit(0)
...
func applyShortFlags(flags string, opts *sha384sumOptions) error {
	for _, c := range flags {
		switch c {
		case 'b':
			opts.binary = true
...
func printHelp() {
	fmt.Print(`Usage: sha384sum [OPTION]... [FILE]...
