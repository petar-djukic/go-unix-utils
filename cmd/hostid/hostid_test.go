...
// Implements: prd048-hostid R3.1, R3.2, R3.3 (differential tests)
package main
...
// refBinaryName is the Homebrew GNU reference binary for hostid.
const refBinaryName = "ghostid"
...
func TestDiff(t *testing.T) {
	...
	programNamePattern := regexp.MustCompile(`(?:` + regexp.QuoteMeta(refBin) + `|ghostid|hostid)`)
	...
	tests := []testutils.DiffTest{
		// R3.2: normal invocation — prints host identifier, exit 0.
		{
			Name: "no_args",
			Args: []string{},
			Env:  []string{"LC_ALL=C"},
