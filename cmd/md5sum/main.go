...
// Package main implements the md5sum utility for computing and verifying MD5 digests.
//
// Implements prd030-md5sum: digest computation (R1), checksum verification (R2),
	...
	for _, arg := range args {
		...
		if strings.HasPrefix(arg, "--") {
			...
			case "--strict":
				f.strict = true
			default:
				fmt.Fprintf(os.Stderr, "md5sum: unrecognized option '%s'\n", arg)
				os.Exit(1)
			}
			continue
		}
		...
		if arg[0] == '-' {
			for _, ch := range arg[1:] {
				...
				case 'w':
					f.warn = true
				default:
					fmt.Fprintf(os.Stderr, "md5sum: invalid option -- '%c'\n", ch)
					os.Exit(1)
				}
			}
			continue
...
// runCompute computes MD5 digests for files or stdin. R1.
...
// computeHash computes the MD5 hex digest of a file or stdin ("-").
...
func runCheck(f flags, files []string) int {
	if len(files) == 0 {
		fmt.Fprintf(os.Stderr, "md5sum: --check requires a file argument\n")
		return 1
	}
...
func checkFile(f flags, name string) int {
	...
	if failures > 0 {
		if !f.status {
			fmt.Fprintf(os.Stderr, "md5sum: WARNING: %d computed checksum did NOT match\n", failures)
		}
		return 1
	}
...
// or BSD tag format ("MD5 (FILENAME) = HASH"). R2.1.
func parseChecksumLine(line string) (hash, filename string, ok bool) {
	...
	if len(line) < 34 { // 32 hex + separator + at least 1 char filename
		return "", "", false
	}
