...
// Package main implements GNU md5sum: compute and check MD5 message digests.
// Implements prd030-md5sum R1-R4.
package main
...
// runHash computes and prints MD5 digests for the given files. (prd030-md5sum R1)
...
// computeHash reads a file (or stdin for "-") and returns its MD5 hex digest. (prd030-md5sum R1.1, R1.2, D1)
...
// printHash writes a single hash line to the writer. (prd030-md5sum R1.1, R1.3, R3.1, R3.2)
func printHash(w *bufio.Writer, hash, name string, opts options) {
	...
	if opts.tag {
		// R1.3: BSD-style "MD5 (FILENAME) = HASH"
...
// runCheck verifies checksums from one or more checksum files. (prd030-md5sum R2)
...
// checkFile reads a checksum file and verifies each entry. (prd030-md5sum R2.1, R2.2, R2.3, R2.4)
...
// ("HASH  FILENAME" or "HASH *FILENAME") and BSD tag format ("MD5 (FILENAME) = HASH").
// (prd030-md5sum R2.1)
func parseCheckLine(line string) (hash, filename string, ok bool) {
	// Try BSD tag format: "MD5 (FILENAME) = HASH"
	...
	if len(line) < md5.Size*2+2 {
		return "", "", false
	}

	hash = line[:md5.Size*2]
	sep := line[md5.Size*2 : md5.Size*2+2]

	if sep != "  " && sep != " *" {
		return "", "", false
	}

	filename = line[md5.Size*2+2:]
	if filename == "" {
		return "", "", false
	}
