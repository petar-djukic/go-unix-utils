...
// Tests for cmd/md5sum implementing prd030-md5sum R1.1-R1.4, R2.1-R2.4,
// R3.1-R3.3, R4.1-R4.3.
package main
...
// TestDiff runs differential tests against the gmd5sum reference binary.
func TestDiff(t *testing.T) {
	goBin := testutils.BuildBinary(t, ".")
	refBin, err := exec.LookPath("gmd5sum")
	if err != nil {
		t.Skip("reference binary gmd5sum not in PATH")
	}
	...
	tests := []testutils.DiffTest{
		...
		// R3.2: -t text mode (default) uses "HASH  FILENAME" format.
		{
		...
		// R1.3: --tag outputs BSD format "MD5 (FILENAME) = HASH".
		{
			Name:     "tag_format",
		...
		// R3.3: --tag with -b still uses BSD tag format.
		{
		...
		// R1.2: empty stdin produces the MD5 of empty input.
		{
			Name:     "empty_stdin",
			Args:     []string{},
			Stdin:    []byte{},
...
func TestDiffCheckFailed(t *testing.T) {
	...
	tests := []testutils.DiffTest{
		...
		// Normalize: stderr summary format differs between Go and GNU.
		{
			Name:      "check_failed",
			Args:      []string{"-c", checksumFile},
			Env:       []string{"LC_ALL=C"},
		...
		// R2.4: --status with mismatch exits 1, no output.
		{
...
func TestDiffNonexistentFile(t *testing.T) {
	...
	tests := []testutils.DiffTest{
		...
		// Normalize: stderr error message format differs between Go and GNU.
		{
...
func TestDiffCheckWarn(t *testing.T) {
	...
	tests := []testutils.DiffTest{
		...
		// Normalize: stderr warning format differs between Go and GNU.
		{
			Name:      "check_warn",
