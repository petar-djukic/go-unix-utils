...
// Package main implements the sha512sum utility for computing and verifying SHA-512 digests.
//
// Implements prd033-sha512sum: digest computation (R1), checksum verification (R2),
...
// hashLength is the length of a SHA-512 hex digest string.
	...
	for _, arg := range args {
		...
		if strings.HasPrefix(arg, "--") {
			...
			case "--strict":
				f.strict = true
			default:
				fmt.Fprintf(os.Stderr, "sha512sum: unrecognized option '%s'\n", arg)
				os.Exit(1)
			}
			continue
		}
		...
		if arg[0] == '-' {
			for _, ch := range arg[1:] {
				...
				case 'w':
					f.warn = true
				default:
					fmt.Fprintf(os.Stderr, "sha512sum: invalid option -- '%c'\n", ch)
					os.Exit(1)
				}
			}
			continue
...
// runCompute computes SHA-512 digests for files or stdin. R1.
...
// computeHash computes the SHA-512 hex digest of a file or stdin ("-").
...
func runCheck(f flags, files []string) int {
	if len(files) == 0 {
		fmt.Fprintf(os.Stderr, "sha512sum: --check requires a file argument\n")
		return 1
	}
...
func checkFile(f flags, name string) int {
	...
	if failures > 0 {
		if !f.status {
			fmt.Fprintf(os.Stderr, "sha512sum: WARNING: %d computed checksum did NOT match\n", failures)
		}
		return 1
	}
...
// or BSD tag format ("SHA512 (FILENAME) = HASH"). R2.1.
