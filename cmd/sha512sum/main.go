...
// Package main implements GNU sha512sum: compute and check SHA-512 message digests.
// Implements prd033-sha512sum R1-R4.
package main
...
// runHash computes and prints SHA-512 digests for the given files. (prd033-sha512sum R1)
...
// computeHash reads a file (or stdin for "-") and returns its SHA-512 hex digest. (prd033-sha512sum R1.1, R1.2)
...
// printHash writes a single hash line to the writer. (prd033-sha512sum R1.1, R1.3, R3.1, R3.2)
func printHash(w *bufio.Writer, hash, name string, opts options) {
	...
	if opts.tag {
		// R1.3: BSD-style "SHA512 (FILENAME) = HASH"
...
// runCheck verifies checksums from one or more checksum files. (prd033-sha512sum R2)
...
// checkFile reads a checksum file and verifies each entry. (prd033-sha512sum R2.1, R2.2, R2.3)
...
// ("HASH  FILENAME" or "HASH *FILENAME") and BSD tag format ("SHA512 (FILENAME) = HASH").
// (prd033-sha512sum R2.1)
func parseCheckLine(line string) (hash, filename string, ok bool) {
	// Try BSD tag format: "SHA512 (FILENAME) = HASH"
	...
	if len(line) < sha512.Size*2+2 {
		return "", "", false
	}

	hash = line[:sha512.Size*2]
	sep := line[sha512.Size*2 : sha512.Size*2+2]

	if sep != "  " && sep != " *" {
		return "", "", false
	}

	filename = line[sha512.Size*2+2:]
	if filename == "" {
		return "", "", false
	}
