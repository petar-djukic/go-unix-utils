...
func TestDiff(t *testing.T) {
	...
	helloHash := sha512hex([]byte("hello\n"))
	emptyHash := sha512hex([]byte{})
	fakeHash := strings.Repeat("0", 128)

	validChecksum := filepath.Join(tmpDir, "valid.sha512")
	writeFile(t, validChecksum, fmt.Sprintf("%s  %s\n", helloHash, helloFile))

	multiChecksum := filepath.Join(tmpDir, "multi.sha512")
	writeFile(t, multiChecksum, fmt.Sprintf("%s  %s\n%s  %s\n", helloHash, helloFile, emptyHash, emptyFile))

	mismatchChecksum := filepath.Join(tmpDir, "mismatch.sha512")
	writeFile(t, mismatchChecksum, fmt.Sprintf("%s  %s\n", fakeHash, helloFile))

	mixedChecksum := filepath.Join(tmpDir, "mixed.sha512")
	writeFile(t, mixedChecksum, fmt.Sprintf("%s  %s\n%s  %s\n", helloHash, helloFile, fakeHash, emptyFile))

	malformedChecksum := filepath.Join(tmpDir, "malformed.sha512")
	writeFile(t, malformedChecksum, fmt.Sprintf("this is not a checksum\n%s  %s\n", helloHash, helloFile))

	bsdChecksum := filepath.Join(tmpDir, "bsd.sha512")
	writeFile(t, bsdChecksum, fmt.Sprintf("SHA512 (%s) = %s\n", helloFile, helloHash))
	...
	tests := []testutils.DiffTest{
		...
		// R4.2: exit 1 when the checksum file is unreadable
		{
			...
			Args:      []string{"--check", filepath.Join(tmpDir, "nonexistent.sha512")},
			ExitCode:  1,
			Normalize: []testutils.NormalizeFunc{discardStderr},
		},
...
func TestSIGPIPE(t *testing.T) {
	...
	if ctx.Err() == context.DeadlineExceeded {
		t.Fatal("sha512sum timed out; SIGPIPE handler may not be installed")
	}
	if err != nil {
