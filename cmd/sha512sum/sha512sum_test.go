...
// TestDiff runs differential tests against the gsha512sum reference binary.
func TestDiff(t *testing.T) {
	...
	tests := []testutils.DiffTest{
		...
		// R1.3: --tag outputs BSD format "SHA512 (FILENAME) = HASH".
		{
			Name:     "tag_format",
		...
		// R1.2: empty stdin produces the SHA-512 of empty input.
		{
			Name:     "empty_stdin",
			Args:     []string{},
			Stdin:    []byte{},
...
//
// R2.3, R3.1: -w warns about improperly formatted checksum lines on stderr.
