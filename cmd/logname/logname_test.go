...
// Implements: prd053-logname R3.1, R3.2, R3.3 (differential tests)
package main
...
// refBinaryName is the Homebrew GNU reference binary for logname.
const refBinaryName = "glogname"
...
func TestDiff(t *testing.T) {
	...
	programNamePattern := regexp.MustCompile(`(?:` + regexp.QuoteMeta(refBin) + `|glogname|logname)`)
	...
	tests := []testutils.DiffTest{
		// R3.2: normal invocation — prints login name, exit 0.
		{
			Name: "no_args",
			Args: []string{},
			Env:  []string{"LC_ALL=C"},
