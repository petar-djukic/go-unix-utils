...
// Differential tests for prd053-logname R3.1 (compare Go vs glogname),
...
// stderrNormalizer normalizes stderr differences between Go and GNU logname:
// replaces binary name/path variants with "logname" and strips
// "Try ... for more information." lines.
var stderrNormalizer testutils.NormalizeFunc = func(data []byte) []byte {
	nameRe := regexp.MustCompile(`[^\s']*g?logname`)
	data = nameRe.ReplaceAll(data, []byte("logname"))
...
func TestDiff(t *testing.T) {
	...
	tests := []testutils.DiffTest{
		// R3.1, R3.2: default invocation — prints login name.
		{
		...
		// R3.2 edge case: double-dash alone — treated as no args, prints login name.
		{
