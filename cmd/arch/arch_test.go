...
// Differential tests for cmd/arch against GNU garch.
// Covers prd045-arch R3.1-R3.2 (differential testing, test coverage).
package main
...
func helpVersionNormalizer() testutils.NormalizeFunc {
	...
	binPath := regexp.MustCompile(`(?m)/[^\s]+/g?arch`)
	versionLine := regexp.MustCompile(`(?m)^arch \([^)]+\) .+$`)
	...
	return func(b []byte) []byte {
		...
		b = binPath.ReplaceAll(b, []byte("arch"))
		b = versionLine.ReplaceAll(b, []byte("arch (NORMALIZED) VERSION"))
...
func stderrNormalizer() testutils.NormalizeFunc {
	binPath := regexp.MustCompile(`/[^\s:]+/g?arch|garch`)
