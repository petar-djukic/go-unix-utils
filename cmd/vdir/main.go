...
// Package main implements cmd/vdir: list directory contents verbosely.
// vdir is equivalent to ls -l -b: long format with C-style escaping.
...
const (
	fmtLong    formatMode = iota // R1.1: vdir defaults to long format
...
// vdirEntry holds a directory entry name and optional metadata.
...
func applyFlag(opts *options, ch rune) error {
	...
	case 'b':
		// R1.2: -b is the default for vdir; accept but no-op.
	case 'd':
		opts.dirOnly = true
	case 'h':
...
func sortEntries(entries []vdirEntry, opts *options) {
...
func selectLess(entries []vdirEntry, mode sortMode) func(i, j int) bool {
	switch mode {
	case sortTime:
...
func timeBeforeName(a, b vdirEntry) bool {
...
func sizeBeforeName(a, b vdirEntry) bool {
	if a.fi != nil && b.fi != nil {
...
// resolveEntries builds vdirEntry items with optional metadata.
func resolveEntries(dir string, names []string, info bool) []vdirEntry {
	entries := make([]vdirEntry, 0, len(names))
	for _, name := range names {
		e := vdirEntry{name: name}
		if info {
...
func displayNames(entries []vdirEntry, classify bool) []string {
...
func buildPrefixedNames(entries []vdirEntry, opts *options) []string {
...
func entryPrefix(e vdirEntry, opts *options, inodeW, blocksW int) string {
	var prefix string
	if opts.showInode {
		ino := "0"
		if e.fi != nil {
...
func computePrefixWidths(entries []vdirEntry, opts *options) (int, int) {
	inodeW, blocksW := 0, 0
	for _, e := range entries {
		if e.fi == nil {
			continue
...
// tabSize is the default tab stop size matching GNU ls/dir/vdir.
const tabSize = 8

// printEntries outputs entries in the appropriate format.
func printEntries(entries []vdirEntry, opts *options) {
	if len(entries) == 0 {
		return
	}
...
func printLong(entries []vdirEntry, dir string, opts *options, showTotal bool) {
	long := buildLongEntries(entries, dir, opts)
	cols := computeLongWidths(long, opts)
	if showTotal {
		printTotalLine(entries, opts)
	}
	for _, le := range long {
		printLongLine(le, cols, opts)
	}
}

// buildLongEntries pre-formats fields for each entry.
func buildLongEntries(entries []vdirEntry, dir string, opts *options) []longEntry {
...
// toLongEntry converts one vdirEntry to a formatted longEntry.
func toLongEntry(e vdirEntry, dir string, opts *options) longEntry {
	le := longEntry{
...
// R1.2: applies C-style escaping to filename.
func longDisplayName(e vdirEntry, dir string, classify bool) string {
	escaped := escapeC(e.name)
...
func printLongLine(e longEntry, w longWidths, opts *options) {
	prefix := longLinePrefix(e, w)
	fields := buildLongFields(e, w, opts)
	fmt.Printf("%s%s\n", prefix, strings.Join(fields, " "))
}

// buildLongFields assembles the visible fields for one long-format line.
func buildLongFields(e longEntry, w longWidths, opts *options) []string {
	fields := []string{
		e.perm,
		format.PadLeft(e.nlink, w.nlink),
	}
	if !opts.hideOwner {
		fields = append(fields, format.PadRight(e.owner, w.owner))
	}
	if !opts.hideGroup {
		fields = append(fields, format.PadRight(e.group, w.group))
	}
...
// R1.3: N = sum(fi.Blocks) / 2 (converts 512-byte to 1K units).
func printTotalLine(entries []vdirEntry, opts *options) {
	var total int64
	for _, e := range entries {
		if e.fi != nil {
			total += e.fi.Blocks
...
func collectSubdirs(dir string, entries []vdirEntry) []string {
	var dirs []string
	for _, e := range entries {
		if e.name == "." || e.name == ".." {
			continue
...
func listFileArgs(files []string, opts *options, exitCode *int) {
	entries := make([]vdirEntry, 0, len(files))
	for _, f := range files {
		fi, err := sys.Lstat(f)
		if err != nil {
		...
		entries = append(entries, vdirEntry{name: f, fi: fi})
	}
	sortEntries(entries, opts)
	if len(entries) == 0 {
		return
...
// formatError produces GNU vdir-compatible error messages.
...
// run executes the vdir command and returns the exit code.
...
func main() {
	...
	// R1.5: default to current directory.
	if len(paths) == 0 {
		paths = []string{"."}
	}
