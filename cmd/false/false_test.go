...
// Differential tests for cmd/false against GNU gfalse.
// Covers prd014-false R3.1-R3.2 (exit codes), R4.1-R4.3 (differential testing).
package main
...
func helpVersionNormalizer() testutils.NormalizeFunc {
	...
	// Normalize full binary paths to just "false" in Usage lines.
	binPath := regexp.MustCompile(`(?m)/[^\s]+/g?false`)
	// Matches "false (GNU coreutils) X.Y.Z" or "false (go-unix-utils) dev".
	versionLine := regexp.MustCompile(`(?m)^false \([^)]+\) .+$`)
	...
	return func(b []byte) []byte {
		...
		b = binPath.ReplaceAll(b, []byte("false"))
		b = versionLine.ReplaceAll(b, []byte("false (NORMALIZED) VERSION"))
...
func TestDiff(t *testing.T) {
	...
	tests := []testutils.DiffTest{
		// R4.1, R4.3: no arguments — exit 1, no output.
		{
			Name: "no_args",
		},
		// R4.2: arbitrary arguments — exit 1, no output.
		{
		...
		// R4.2: single dash argument — exit 1, no output.
		{
			Name: "dash_arg",
			Args: []string{"-"},
		},
		// R4.2: flags that look like options — still exit 1, no output.
		{
		...
		// R3.1: --help not first arg — treated as ignored arg, exit 1.
		{
			Name: "help_not_first",
			Args: []string{"foo", "--help"},
		},
		// R3.1: --version not first arg — treated as ignored arg, exit 1.
		{
