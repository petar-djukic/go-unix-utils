...
// Implements prd004-ts R1.1-R1.4: cmd/ts reads stdin line by line and prepends
// a strftime-formatted timestamp to each line, writing to stdout. Supports a
// default format ("%b %d %H:%M:%S") and an optional positional format argument.
...
func main() {
	...
	// R1.3: accept an optional positional argument specifying a strftime format.
	args := os.Args[1:]
	if len(args) > 0 {
		format = args[0]
	}
