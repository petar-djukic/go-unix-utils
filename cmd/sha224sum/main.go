...
// Package main implements cmd/sha224sum: compute and check SHA-224 message digests.
// Implements srd074-sha224sum R1.1, R1.2, R1.3, R1.4, R2.1, R2.2, R2.3, R3.1, R3.2,
// R4.1, R4.2, R4.3.
package main
...
const (
	...
	// R1.2: SHA-224 produces 28-byte (56 hex character) digests.
	sha224DigestLen = 56
)

// usageText is the --help output printed to stdout.
const usageText = `Usage: sha224sum [OPTION]... [FILE]...
...
const versionText = "sha224sum (go-unix-utils) 0.1.0\n"

// config holds parsed command-line options for sha224sum.
type config struct {
...
// run executes the sha224sum logic and returns the exit code.
...
func run(cfg config) int {
	...
	// R1.2: configure HashConfig with SHA224 algorithm.
	hcfg := hashutil.HashConfig{
		Algorithm: algorithmName,
		NewHash:   sha256.New224,
		DigestLen: sha224DigestLen,
	}
