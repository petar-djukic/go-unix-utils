...
// Implements prd032-sha256sum R1.1, R1.2, R1.3, R1.4: SHA-256 digest computation
// with GNU and BSD tag output formats.
// Implements prd032-sha256sum R2.1, R2.2, R2.3: checksum verification
// with --check, --warn, --quiet, --status.
// Implements prd032-sha256sum R3.1, R3.2: binary/text mode flags and
// tag format interaction.
// Implements prd032-sha256sum R4.1, R4.2, R4.3: exit codes and SIGPIPE handling.
package main
...
var gnuPattern = regexp.MustCompile(`^([0-9a-fA-F]{64}) [ *](.+)$`)

// bsdPattern matches BSD tag format: "SHA256 (FILENAME) = HASH".
var bsdPattern = regexp.MustCompile(`^SHA256 \((.+)\) = ([0-9a-fA-F]{64})$`)
...
// processFile computes the SHA-256 digest for a single file or stdin and
...
// computeDigest reads the file (or stdin for "-") and returns its SHA-256 hex digest.
...
// printError formats an error message matching GNU sha256sum style.
...
// R1.2: stdin shown as "-".
// R1.3: BSD tag format "SHA256 (FILENAME) = HASH".
		...
		if !ok {
			...
			if opts.warn {
				...
				fmt.Fprintf(os.Stderr,
					"sha256sum: %s: %d: improperly formatted SHA256 checksum line\n",
					name, lineNum)
			}
			continue
		}
...
func printSummaryWarnings(result verifyResult, opts options) {
	if !opts.status && result.malformedCount > 0 {
		...
		fmt.Fprintf(os.Stderr,
			"sha256sum: WARNING: %d %s improperly formatted\n",
	...
	if !opts.status && result.failedCount > 0 {
		...
		fmt.Fprintf(os.Stderr,
			"sha256sum: WARNING: %d %s NOT match\n",
			result.failedCount, noun)
	}
}

// parseLine tries GNU then BSD format. Returns hash, filename, ok.
// R2.1: Supports "HASH  FILENAME", "HASH *FILENAME", and "SHA256 (FILENAME) = HASH".
