...
// Package main implements cmd/sha256sum (prd032-sha256sum R1–R4).
package main
...
func main() {
	...
	if version {
		fmt.Println("sha256sum (Go coreutils) 1.0")
		os.Exit(0)
	}
...
func check(files []string, warn, quiet, status, strict, ignoreMissing bool) int {
	...
	if total > 0 && !status {
		...
		fmt.Fprintf(os.Stderr, "sha256sum: WARNING: %d computed checksum%s did NOT match\n", total, s)
	}
