...
// Package main implements GNU sha256sum: compute and check SHA-256 message digests.
// Implements prd032-sha256sum R1-R4.
package main
...
// runHash computes and prints SHA-256 digests for the given files. (prd032-sha256sum R1)
...
// computeHash reads a file (or stdin for "-") and returns its SHA-256 hex digest. (prd032-sha256sum R1.1, R1.2, D1)
...
// printHash writes a single hash line to the writer. (prd032-sha256sum R1.1, R1.3, R3.1, R3.2)
func printHash(w *bufio.Writer, hash, name string, opts options) {
	...
	if opts.tag {
		// R1.3: BSD-style "SHA256 (FILENAME) = HASH"
...
// runCheck verifies checksums from one or more checksum files. (prd032-sha256sum R2)
...
// checkFile reads a checksum file and verifies each entry. (prd032-sha256sum R2.1, R2.2, R2.3, R2.4)
...
// ("HASH  FILENAME" or "HASH *FILENAME") and BSD tag format ("SHA256 (FILENAME) = HASH").
// (prd032-sha256sum R2.1)
func parseCheckLine(line string) (hash, filename string, ok bool) {
	...
	hash = line[:sha256.Size*2]
	sep := line[sha256.Size*2 : sha256.Size*2+2]
