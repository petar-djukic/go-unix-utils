...
func main() {
	...
	// R2.3: --quiet suppresses OK lines; --status suppresses all output.
...
// hashReader computes the SHA-256 digest of r and prints the result line to stdout.
// R1.1: GNU format "HASH  FILENAME" (text) or "HASH *FILENAME" (binary).
// R1.3: BSD tag format "SHA256 (FILENAME) = HASH" when tag is true.
...
// error messages matching GNU sha256sum format.
...
type checkOpts struct {
	...
	quiet  bool // R2.3: suppress OK lines.
	status bool // R2.3: suppress all output; exit code conveys result.
}

// gnuLineRe matches GNU format: 64 hex chars followed by two spaces (text) or
// space+asterisk (binary), then the filename.
var gnuLineRe = regexp.MustCompile(`^([0-9a-fA-F]{64}) ([ *])(.+)$`)

// bsdTagRe matches BSD tag format: "SHA256 (FILENAME) = HASH".
var bsdTagRe = regexp.MustCompile(`^SHA256 \((.+)\) = ([0-9a-fA-F]{64})$`)
...
// computeFileHash computes the SHA-256 digest of the named file, returning the
...
// R2.3: --quiet suppresses OK; --status suppresses all stdout and stderr output.
func checkChecksums(r io.Reader, source string, opts checkOpts) int {
	...
	// R2.3: exit 1 when any checksum fails or any file could not be read.
