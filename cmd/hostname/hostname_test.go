...
// Differential tests for cmd/hostname against GNU ghostname.
// Covers prd047-hostname R3.1-R3.2 (differential testing, test coverage).
package main
...
func helpVersionNormalizer() testutils.NormalizeFunc {
	...
	binPath := regexp.MustCompile(`(?m)/[^\s]+/g?hostname`)
	versionLine := regexp.MustCompile(`(?m)^hostname \([^)]+\) .+$`)
	...
	return func(b []byte) []byte {
		...
		b = binPath.ReplaceAll(b, []byte("hostname"))
		b = versionLine.ReplaceAll(b, []byte("hostname (NORMALIZED) VERSION"))
...
func stderrNormalizer() testutils.NormalizeFunc {
	binPath := regexp.MustCompile(`/[^\s:]+/g?hostname|ghostname`)
...
func TestDiff(t *testing.T) {
	...
	tests := []testutils.DiffTest{
		// R3.2: normal invocation — no arguments, prints system hostname.
		{
			Name: "no_args",
			Env:  []string{"LC_ALL=C"},
		},
