...
// Differential tests for cmd/hostname against ghostname (GNU coreutils).
// Implements prd047-hostname R3.1-R3.3 test coverage.
package main
...
var binaryPathNormalizer = func(b []byte) []byte {
	re := regexp.MustCompile(`[^\s']*[/\\]?[gG]?hostname`)
...
func TestDiff(t *testing.T) {
	...
	tests := []testutils.DiffTest{
		// R1.1, R3.1, R3.2: no arguments — prints system hostname.
		{
			Name:     "R1.1_no_args_hostname",
			Env:      lcEnv,
			ExitCode: 0,
		},
