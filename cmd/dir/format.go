...
// format.go implements output formatting for cmd/dir (prd107-dir R1.5):
// sorting, multi-column, single-column, across, and long-format output
...
func computePrefixWidths(cfg *dirConfig, entries []dirEntry) entryPrefixWidths {
...
func formatEntryPrefix(cfg *dirConfig, e dirEntry, pw entryPrefixWidths) string {
	if !cfg.showInode && !cfg.showBlocks {
		return ""
	}
...
func entryNames(cfg *dirConfig, entries []dirEntry, pw entryPrefixWidths) []string {
...
func formatSingleColumn(cfg *dirConfig, entries []dirEntry, pw entryPrefixWidths) {
	for _, e := range entries {
		fmt.Printf("%s%s\n",
...
func formatMultiColumn(cfg *dirConfig, entries []dirEntry, pw entryPrefixWidths) {
...
func formatAcrossColumns(cfg *dirConfig, entries []dirEntry, pw entryPrefixWidths) {
	names := entryNames(cfg, entries, pw)
...
func formatLongListing(cfg *dirConfig, entries []dirEntry) {
...
func computeColumnWidths(cfg *dirConfig, entries []dirEntry) columnWidths {
	var cw columnWidths
	for _, e := range entries {
		if e.info == nil {
			continue
...
func printLongEntry(cfg *dirConfig, e dirEntry, cw columnWidths) {
	if e.info == nil {
...
func longEntryPrefix(cfg *dirConfig, fi *sys.FileInfo, cw columnWidths) string {
	if !cfg.showInode && !cfg.showBlocks {
		return ""
	}
	var sb strings.Builder
...
func printTotalLine(cfg *dirConfig, entries []dirEntry) {
...
func entryDisplayName(cfg *dirConfig, e dirEntry) string {
...
func coloredEscapedName(e dirEntry) string {
	escaped := escapeName(e.name)
	if e.info != nil {
		c := format.FileTypeColor(e.info.Mode)
		if c != "" {
