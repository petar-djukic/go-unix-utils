...
// Implements prd107-dir R1.1-R1.5, R2.1-R2.4: directory listing equivalent
// to ls -C -b. Accepts all ls flags with multi-column format and C-style
...
func applyColorMode(cfg *dirConfig) {
	switch cfg.color {
	case colorAlways:
...
// R1.5: accepts all ls short flags.
// R2.3: exits 2 for unrecognized options.
func applyShortFlag(ch rune, cfg *dirConfig) {
	switch ch {
	case 'a':
...
func handleLongFlag(arg string, cfg *dirConfig) bool {
	switch arg {
	case "--all":
		cfg.filter = filterAll
	case "--almost-all":
...
func handleLongFlagValue(arg string, cfg *dirConfig) bool {
...
func applyColorValue(val string, cfg *dirConfig) {
	switch val {
...
func applyTimeValue(val string, cfg *dirConfig) {
	switch val {
...
// R2.2: command-line access failures set exit code 2 (matching gdir).
func runDirOnly(cfg dirConfig) int {
...
func dirOnlyStatFunc(cfg dirConfig) func(string) (*sys.FileInfo, error) {
...
// R2.2: command-line access failures set exit code 2 (matching gdir).
func runNormal(cfg dirConfig) int {
	exitCode := 0
...
func listDirs(dirs []entry, cfg dirConfig, needBlank, showHeader bool) int {
	exitCode := 0
	for _, de := range dirs {
		if needBlank {
			fmt.Println()
...
func needsStat(cfg dirConfig) bool {
...
func statEntriesInPlace(entries []entry, cfg dirConfig) {
	sf := sys.Lstat
	if cfg.derefAll {
		sf = sys.Stat
	}
...
func sortEntries(entries []entry, cfg dirConfig) {
...
func selectComparator(cfg dirConfig) func(a, b entry) bool {
	switch cfg.sorting {
	case sortTime:
		tf := cfg.timeSelect
...
func recurseSubdirs(entries []entry, cfg dirConfig) int {
	exitCode := 0
...
func isRealDir(e entry, cfg dirConfig) bool {
