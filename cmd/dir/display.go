...
// Implements prd107-dir display logic: long format output, columnar layout
// with tab-based alignment, C-style filename escaping, permission strings,
// time formatting, and comparison functions. R1.2: C-style escaping is the
...
func displayEntries(entries []entry, cfg dirConfig) int {
...
func displayDirEntries(entries []entry, cfg dirConfig) int {
	if cfg.output == modeLong {
...
func printTotalLine(entries []entry, cfg dirConfig) {
...
func printNonLongEntries(entries []entry, cfg dirConfig) {
	if len(entries) == 0 {
		return
	}
...
func buildDisplayNames(entries []entry, cfg dirConfig, pw prefixWidths) []string {
...
func buildDisplayName(e entry, cfg dirConfig, pw prefixWidths) string {
	var parts []string
	if cfg.showInode {
...
func computePrefixWidths(entries []entry, cfg dirConfig) prefixWidths {
	var pw prefixWidths
	for _, e := range entries {
		if cfg.showInode {
...
// R1.1: dir defaults to multi-column format regardless of TTY.
func resolveOutputMode(cfg dirConfig) outputMode {
...
// matching GNU coreutils indent() behavior. A tab is used only when a
// tab stop lies strictly between pos and target (checked via the
// to/tabsize > (from+1)/tabsize condition from GNU ls.c).
...
func computeLongWidths(entries []entry, cfg dirConfig) longWidths {
	var w longWidths
	for _, e := range entries {
		if e.info == nil {
			continue
...
func printLongLine(e entry, cfg dirConfig, lw longWidths, pw prefixWidths) {
	var prefix string
	if cfg.showInode {
