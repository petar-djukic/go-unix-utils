...
// Implements prd013-true: Exit Successfully.
...
func main() {
	...
	// R1.1-R1.3: exit 0 unconditionally, ignoring all arguments.
}
...
func printHelp() int {
	_, err := fmt.Fprint(os.Stdout, `Usage: true [ignored command line arguments]
  or:  true OPTION
...
func printVersion() int {
	_, err := fmt.Fprintf(os.Stdout, "true (go-unix-utils) %s\n", version)
	if err != nil {
		return 1
	}
	return 0
