...
// Implements: prd042-whoami R3.1, R3.2, R3.3 (differential tests)
package main
...
// refBinaryName is the Homebrew GNU reference binary for whoami.
const refBinaryName = "gwhoami"
...
func TestDiff(t *testing.T) {
	...
	programNamePattern := regexp.MustCompile(`(?:` + regexp.QuoteMeta(refBin) + `|gwhoami|whoami)`)
	...
	tests := []testutils.DiffTest{
		// R3.2, R3.3: no-argument invocation — prints effective username.
		{
			Name: "no_args",
			Args: []string{},
			Env:  []string{"LC_ALL=C"},
