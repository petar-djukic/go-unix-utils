...
// Differential tests for cmd/sha1sum against the GNU reference binary (gsha1sum).
// Implements prd031-sha1sum R1-R4 verification.
package main
...
// normalizeProgramName replaces "gsha1sum:" with "sha1sum:" in stderr output so that
...
func TestDiff(t *testing.T) {
	...
	writeFile(t, dirCheckOK, "checksums.sha1", "f572d396fae9206628714fb2ce00f72e94f2258f  hello.txt\n")

	// Setup: directory with invalid checksum file
	dirCheckBad := t.TempDir()
	writeFile(t, dirCheckBad, "hello.txt", "hello\n")
	writeFile(t, dirCheckBad, "bad.sha1", "0000000000000000000000000000000000000000  hello.txt\n")
	...
	writeFile(t, dirQuiet, "checksums.sha1", "f572d396fae9206628714fb2ce00f72e94f2258f  hello.txt\n")

	// Setup: directory for --status check test
	dirStatus := t.TempDir()
	writeFile(t, dirStatus, "hello.txt", "hello\n")
	writeFile(t, dirStatus, "checksums.sha1", "f572d396fae9206628714fb2ce00f72e94f2258f  hello.txt\n")
	...
	writeFile(t, dirStatusFail, "bad.sha1", "0000000000000000000000000000000000000000  hello.txt\n")
