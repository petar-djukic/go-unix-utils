...
var warnLinePattern = regexp.MustCompile(`(?:g?sha1sum): [^:]+: \d+: improperly formatted[^\n]*\n`)

// warnNormalizer normalizes warning lines about malformed checksum lines.
func warnNormalizer(data []byte) []byte {
	return warnLinePattern.ReplaceAll(data, []byte("sha1sum: WARN_LINE\n"))
}

// failedSummaryPattern matches the summary line printed after --check.
var failedSummaryPattern = regexp.MustCompile(`(?:g?sha1sum): WARNING: \d+ computed checksum[^\n]*\n`)

// failedSummaryNormalizer normalizes the summary warning line.
func failedSummaryNormalizer(data []byte) []byte {
	return failedSummaryPattern.ReplaceAll(data, []byte("sha1sum: WARNING_SUMMARY\n"))
}

// sha1hex returns known SHA-1 hashes for test data.
func sha1hex(content string) string {
	switch content {
	case "hello world\n":
		return "22596363b3de40b06f981fb85d82312e8c0ed511"
	case "test data\n":
		return "f48dd853820860816c75d54d0f584dc863327a7c"
	default:
		return ""
	}
}
...
func TestDiff(t *testing.T) {
	...
	tests := []testutils.DiffTest{
		...
		// R3.1: -t flag is default text mode, "HASH  FILENAME".
		{
...
func setupCheckFailDir(t *testing.T) string {
	...
	checksumLine := "0000000000000000000000000000000000000000  input.txt\n"
	writeTestFile(t, dir, "checksums.txt", checksumLine)
	return dir
}
...
func setupCheckBSDDir(t *testing.T) string {
	...
	writeTestFile(t, dir, "input.txt", "hello world\n")
	hash := sha1hex("hello world\n")
	content := fmt.Sprintf("SHA1 (input.txt) = %s\n", hash)
	writeTestFile(t, dir, "checksums.txt", content)
	return dir
}
