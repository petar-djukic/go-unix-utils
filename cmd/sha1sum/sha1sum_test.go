...
// Package main provides differential tests for cmd/sha1sum against gsha1sum.
// Implements srd031-sha1sum R4.1, R4.2, R4.3 acceptance criteria via
...
// comparison succeeds. Handles "gsha1sum:" and full path forms.
func normalizeStderr(data []byte) []byte {
	data = bytes.ReplaceAll(data, []byte("gsha1sum:"), []byte("sha1sum:"))
	idx := bytes.Index(data, []byte("/sha1sum:"))
	for idx >= 0 {
		...
		if data[start] == '/' {
			data = append(data[:start], append([]byte("sha1sum:"), data[idx+len("/sha1sum:"):]...)...)
		}
		next := bytes.Index(data[start+8:], []byte("/sha1sum:"))
		if next == -1 {
			break
		}
		idx = start + 8 + next
...
// that GNU sha1sum appends to some error messages.
...
// Our implementation prints "SHA1: file:" while GNU prints "sha1sum: file:".
func normalizeAlgoPrefix(data []byte) []byte {
	data = bytes.ReplaceAll(data, []byte("SHA1: "), []byte("sha1sum: "))
	return data
}

// normalizeWarningPrefix adds "sha1sum: " prefix to bare WARNING lines
// so they match GNU's "sha1sum: WARNING:" format.
func normalizeWarningPrefix(data []byte) []byte {
	data = bytes.ReplaceAll(data,
		[]byte("WARNING: "),
		[]byte("sha1sum: WARNING: "))
	// Fix double-prefix if it was already prefixed.
	data = bytes.ReplaceAll(data,
		[]byte("sha1sum: sha1sum: WARNING:"),
		[]byte("sha1sum: WARNING:"))
	return data
}
...
// TestDiff runs differential tests for sha1sum against gsha1sum.
// D1: uses testutils.BuildBinary and exec.LookPath.
// D2: skips if gsha1sum not found.
func TestDiff(t *testing.T) {
	...
	// Must be exactly 40 hex chars for SHA-1.
	checksumFail := writeTestFile(t, dir, "checksums_fail.txt",
		"000000000000000000000000000000000000dead  "+hello+"\n")

	// Checksum with BSD tag format.
	checksumBSD := writeTestFile(t, dir, "checksums_bsd.txt",
		"SHA1 ("+hello+") = f572d396fae9206628714fb2ce00f72e94f2258f\n")
	...
	checksumMixed := writeTestFile(t, dir, "checksums_mixed.txt",
		"f572d396fae9206628714fb2ce00f72e94f2258f  "+hello+"\n"+
			"000000000000000000000000000000000000dead  "+abc+"\n")
	...
	tests := []testutils.DiffTest{
		...
		{
			// R1.1: empty file produces valid SHA-1 digest.
			Name: "empty_file",
			Args: []string{empty},
		},
		{
