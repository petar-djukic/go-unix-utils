...
// Package main implements the sha1sum utility for computing and verifying SHA-1 digests.
//
// Implements prd031-sha1sum: digest computation (R1), checksum verification (R2),
	...
	for _, arg := range args {
		...
		if strings.HasPrefix(arg, "--") {
			...
			case "--strict":
				f.strict = true
			default:
				fmt.Fprintf(os.Stderr, "sha1sum: unrecognized option '%s'\n", arg)
				os.Exit(1)
			}
			continue
		}
		...
		if arg[0] == '-' {
			for _, ch := range arg[1:] {
				...
				case 'w':
					f.warn = true
				default:
					fmt.Fprintf(os.Stderr, "sha1sum: invalid option -- '%c'\n", ch)
					os.Exit(1)
				}
			}
			continue
...
// runCompute computes SHA-1 digests for files or stdin. R1.
...
// computeHash computes the SHA-1 hex digest of a file or stdin ("-").
...
// hashLength is the length of a SHA-1 hex digest string.
const hashLength = 40

// runCheck reads a checksum file and verifies each entry. R2.
func runCheck(f flags, files []string) int {
	if len(files) == 0 {
		fmt.Fprintf(os.Stderr, "sha1sum: --check requires a file argument\n")
		return 1
	}
...
func checkFile(f flags, name string) int {
	...
	for lineNum := 1; scanner.Scan(); lineNum++ {
		...
		if !ok {
			malformed++
			if f.warn {
				fmt.Fprintf(os.Stderr, "sha1sum: %s: %d: improperly formatted SHA1 checksum line\n", name, lineNum)
			}
			continue
		}
	...
	if failures > 0 {
		if !f.status {
			fmt.Fprintf(os.Stderr, "sha1sum: WARNING: %d computed checksum did NOT match\n", failures)
		}
		return 1
	}

	if checked == 0 && malformed > 0 {
		fmt.Fprintf(os.Stderr, "sha1sum: %s: no properly formatted SHA1 checksum lines found\n", name)
		return 1
	}
...
// or BSD tag format ("SHA1 (FILENAME) = HASH"). R2.1.
