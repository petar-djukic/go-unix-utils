...
var gnuPattern = regexp.MustCompile(`^([0-9a-fA-F]{40}) [ *](.+)$`)

// bsdPattern matches BSD tag format: "SHA1 (FILENAME) = HASH".
var bsdPattern = regexp.MustCompile(`^SHA1 \((.+)\) = ([0-9a-fA-F]{40})$`)
		...
		if !ok {
			...
			if opts.warn {
				...
				fmt.Fprintf(os.Stderr,
					"sha1sum: %s: %d: improperly formatted SHA1 checksum line\n",
					name, lineNum)
			}
			continue
		}
...
func printSummaryWarnings(result verifyResult, opts options) {
	if !opts.status && result.malformedCount > 0 {
		...
		fmt.Fprintf(os.Stderr,
			"sha1sum: WARNING: %d %s improperly formatted\n",
	...
	if !opts.status && result.failedCount > 0 {
		...
		fmt.Fprintf(os.Stderr,
			"sha1sum: WARNING: %d %s NOT match\n",
			result.failedCount, noun)
	}
}

// parseLine tries GNU then BSD format. Returns hash, filename, ok.
// R2.1: Supports "HASH  FILENAME", "HASH *FILENAME", and "SHA1 (FILENAME) = HASH".
