...
// hashReader computes the SHA-1 digest of r and prints the result line to stdout.
// R1.1: GNU format "HASH  FILENAME" (text) or "HASH *FILENAME" (binary).
// R1.3: BSD tag format "SHA1 (FILENAME) = HASH" when tag is true.
...
// error messages matching GNU sha1sum format.
...
// gnuLineRe matches GNU format: 40 hex chars followed by two spaces (text) or
// space+asterisk (binary), then the filename.
var gnuLineRe = regexp.MustCompile(`^([0-9a-fA-F]{40}) ([ *])(.+)$`)

// bsdTagRe matches BSD tag format: "SHA1 (FILENAME) = HASH".
var bsdTagRe = regexp.MustCompile(`^SHA1 \((.+)\) = ([0-9a-fA-F]{40})$`)
...
// computeFileHash computes the SHA-1 digest of the named file, returning the
	...
	for scanner.Scan() {
		...
		if !ok {
			...
			if opts.warn {
				...
				fmt.Fprintf(os.Stderr, "%s: %s: %d: improperly formatted SHA1 checksum line\n",
