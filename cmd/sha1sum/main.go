...
// Implements prd031-sha1sum R1.1-R1.4, R2.1-R2.3, R3.1-R3.2: Compute and
// check SHA-1 message digests using pkg/hashutil for shared formatting and
...
// D4: Algorithm="SHA1", NewHash=sha1.New, DigestLen=40 (hex length).
...
type sha1sumOptions struct {
	...
	strict bool // --strict: exit non-zero on malformed lines (R3.2)
}
...
// D1: delegates to hashutil.DigestFiles for digest mode,
...
func runCheck(opts sha1sumOptions, files []string) int {
	...
	for _, f := range files {
		ok, err := hashutil.VerifyChecksums(
			f, sha1Config, checkOpts, os.Stdout, os.Stderr,
		)
		if err != nil {
...
// buildCheckOptions constructs CheckOptions from sha1sumOptions.
// R2.3: --strict implies --warn behavior per GNU coreutils convention.
func buildCheckOptions(opts sha1sumOptions) hashutil.CheckOptions {
...
// D3: uses manual parsing for both short and long flag forms.
