...
// Package main implements cmd/sha1sum (prd031-sha1sum R1–R4).
package main
...
const usageText = `Usage: sha1sum [OPTION]... [FILE]...
...
func main() {
	...
	if version {
		fmt.Println("sha1sum (Go coreutils) 1.0")
		os.Exit(0)
	}
...
func check(files []string, warn, quiet, status, strict, ignoreMissing bool) int {
	...
	if total > 0 && !status {
		...
		fmt.Fprintf(os.Stderr, "sha1sum: WARNING: %d computed checksum%s did NOT match\n", total, s)
	}
